...
function Library:CreateWindow(options)
	...
	function Window:CreateTab(name)
		...
		--================ TAB: FindToggle =================
		function Tab:FindToggle(name)
			if not self.Elements then return nil end
			for _, element in pairs(self.Elements) do
				if element.Type == "Toggle" and element.Name == name then
					return element:Get()  -- return the current value (true/false)
				end
			end
			return nil
		end
		...
		--================ TAB CONSOLE WITH INPUT & COMMANDS =================
		do
			local Commands = {}
			...
			-- Adds console UI to this tab
			function Tab:AddConsole()
				if ConsoleFrame then return end -- only once

				local Scroll = Instance.new("ScrollingFrame")
				Scroll.Size = UDim2.new(1,-10,0,150)
				Scroll.Position = UDim2.new(0,5,1,-155)
				Scroll.BackgroundColor3 = Settings.Colors.Element
				Scroll.BorderSizePixel = 0
				Scroll.CanvasSize = UDim2.new(0,0,0,0)
				Scroll.ScrollBarImageTransparency = 0.6
				Scroll.Parent = self.Frame -- parent to tab content
				Corner(Scroll,6)
				...
				InputBox.BackgroundColor3 = Settings.Colors.Tab
				InputBox.TextColor3 = Settings.Colors.Text
				InputBox.Font = Settings.Font
				InputBox.TextSize = Settings.TextSize
				InputBox.PlaceholderText = "Enter command..."
				InputBox.Parent = Scroll
				Corner(InputBox,4)
				...
				InputBox.FocusLost:Connect(function(enterPressed)
					...
					if cmd and Commands[cmd] then
						local ok, err = pcall(function() Commands[cmd](args) end)
						if not ok then self:ConsoleError("Command error: "..tostring(err)) end
					else
						self:ConsoleError("Unknown command: "..(cmd or ""))
					end
				end)
			end

			-- Print functions
			local function AddMsgToTab(self,text,color)
				if not ConsoleFrame then return end
				local msg = Instance.new("TextLabel")
				msg.Text = text
				msg.TextColor3 = color or Settings.Colors.Text
				msg.Font = Settings.Font
				msg.TextSize = Settings.TextSize
				msg.BackgroundTransparency = 1
				msg.Size = UDim2.new(1,0,0,20)
				msg.TextXAlignment = Enum.TextXAlignment.Left
				msg.Parent = ConsoleFrame
				ConsoleFrame.CanvasSize = UDim2.new(0,0,0,self.ConsoleLayout.AbsoluteContentSize.Y)
			end

			function Tab:ConsoleLog(msg) AddMsgToTab(self,msg,Settings.Colors.Text) end
			function Tab:ConsoleWarn(msg) AddMsgToTab(self,msg,Settings.Colors.Warn) end
			function Tab:ConsoleInfo(msg) AddMsgToTab(self,msg,Settings.Colors.Info) end
			function Tab:ConsoleError(msg) AddMsgToTab(self,msg,Settings.Colors.Error) end

			-- Bind commands to tab
			function Tab:BindCommand(cmdName, callback)
				if cmdName and callback then
					Commands[cmdName:lower()] = callback
				end
			end
		end
